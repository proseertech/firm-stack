...
description: |
  ...
  present-interest support for ILIT contributions, marital-deduction/QTIP/SLAT
  treatment (including reverse QTIP), DSUE (Schedule C) usage, and adequate
  disclosure for hard-to-value gifts — and sweeps the source documents for
...
- **HIGH** — Wrong Schedule A part classification, missing or defective elections (election-out, §529(c)(2)(B), §2523(f) QTIP, gift-splitting consent), GST allocation that doesn't reconcile, DSUE claimed without Form 706 support or in violation of the last-deceased-spouse limitation, missing adequate-disclosure attachments for hard-to-value gifts, Crummey defects that defeat the annual exclusion, a SLAT accidentally reported as a marital-deduction gift
...
- Predeceased spouse's Form 706 (at least the first four pages, showing the portability election), if DSUE is applied on Schedule C
...
1. **Verify donor information and filing posture** — Donor identity, tax year, individual vs. gift-splitting filing. If gift-splitting is elected: the gift-splitting questions in **Part I (General Information)** are answered, and a **separately signed and dated Notice of Consent is attached**. On the 2024-and-later form revision the consenting spouse no longer signs the return itself — consent lives entirely in the attached statement, which must include the prescribed election language ("[Name of consenting spouse] elects to treat all gifts made by both spouses to third parties during the calendar year as made one-half by each spouse"). CCH Axcess answering the Part I questions is not consent; the signed attachment is. Both spouses' returns must show matching, complementary halved entries (donor's own gifts in the top half of Schedule A Part 1/2/3; "Gifts made by spouse" in the bottom half). Confirm the donor's marital status **at the time of each gift** — §2513 splitting is available only for gifts made while married, so a split election covering pre-marriage or post-divorce gifts is invalid regardless of a perfect consent; if the spouse is a beneficiary of a trust receiving gifts, only the ascertainable non-spouse portion is splittable. If gifts were funded from **community property**, the mechanics are different: each half is automatically a gift by each spouse — two returns, no §2513 consent apparatus — so don't run gift-splitting mechanics on community funds (and where community status is uncertain, splitting can serve as a backstop). If the donor is a nonresident non-citizen, the correct form is **Form 709-NA**, not Form 709.
2. **Tie every gift to source documents** — Each Schedule A entry ties to bank/wire records, checks, premium payments, or trust receipts. Gift descriptions are accurate and complete (trust name, date, EIN, trustee, beneficiaries). Confirm the digital-asset question in Part 1 (General Information) is answered and consistent with the gifts reported — a crypto or NFT gift with a "No" answer is a finding, and any digital-asset gift needs a supportable valuation method (date-of-gift exchange pricing or appraisal) described in the Schedule A entry. If a **life insurance policy** (not cash for premiums) is gifted: confirm the insurer and policy number are described, a completed **Form 712** is attached to support the value, and ask whether the policy sits inside a split-dollar arrangement — economic-benefit split-dollar generates ongoing deemed gifts the return may be silent on. Confirm the **donor's adjusted basis column** is completed (for depreciable property, tie it to depreciation records), and if the donee is paying the gift tax (a **net gift**), confirm the reported value is reduced by the tax assumed and flag the potential income tax consequence to the donor.
3. **Sweep for unreported and deemed gifts** — The tie-out above only validates what is *on* the return; the classic 709 failure is the gift that isn't. Scan the source documents and ask the preparer about: bargain sales to family members (transfers for less than full and adequate consideration), interest-free or below-market loans (§7872), debt forgiveness, lapse or release of a general power of appointment (§2514) outside the Crummey context, entity transactions that shift capital among family members (formations, recapitalizations, disproportionate contributions), and premium payments on life insurance policies the donor does not own. Check §2503(e) transfers in both directions: tuition and medical amounts paid **directly** to the institution or provider are excludable and should not be reported as taxable gifts (reporting them wastes exemption), while "tuition" paid to the student, or room and board, is not excludable and is an unreported gift. An unreported gift has no adequate disclosure — the statute of limitations never starts.
4. **Verify Schedule A part classification** — For every trust/entity gift, confirm direct skip (Part 2) vs. indirect skip (Part 3) vs. non-skip (Part 1) against the actual trust instrument. This is the most common consequential error on a 709. Whether a trust is a "GST trust" under §2632(c)(3)(B) turns on six technical statutory exceptions — read the instrument, don't pattern-match on "trust for a child."
5. **Verify annual exclusions (Schedule D Part 1 and Schedule A Part 4)** — Exclusions match what's actually available per donee (watch split-gift halving). For gifts in trust claiming the exclusion via Crummey powers, work the present-interest analysis in `references/ilit-crummey-review.md` — notice, withdrawal period, lapse mechanics, 5-and-5 / hanging powers.
6. **Verify marital-deduction gifts (QTIP / SLAT / non-citizen spouse)** — For any gift to or in trust for a spouse: outright gifts to a U.S.-citizen spouse generally need no reporting; terminable interests get **no marital deduction** unless a **§2523(f) QTIP election** is made on the return — confirm the election is affirmatively made (listing the trust on Schedule A and deducting it *is* the election; verify it was intended) and the trust qualifies (all income to spouse for life, no other beneficiary during spouse's life). Wherever a lifetime QTIP election is made, check whether a **reverse QTIP election under §2652(a)(3)** was considered and made if intended — without it the spouse becomes the GST transferor and the donor's GST exemption cannot effectively be allocated to the trust; the omission is silent and irrevocable (see `references/gst-mechanics.md`). A **SLAT is deliberately non-QTIP** — confirm it is reported as a taxable gift using lifetime exemption, not accidentally deducted. For a **non-citizen spouse**: no marital deduction; only the enhanced §2523(i) annual exclusion (inflation-adjusted; verify the applicable year's amount) applies, and future-interest gifts get nothing.
7. **Verify §529(c)(2)(B) superfunding elections** — Box checked on Schedule A Part 4 Line B; statement attached with per-beneficiary, per-year breakdown; total doesn't exceed 5× the exclusion actually available (check for other gifts to the same beneficiary that already used room); ratable 1/5 reported in the correct Part (Part 1 for non-skip beneficiary, Part 2 for skip beneficiary). Confirm whether spouses each contributed their own 5× or gift-split one contribution — it changes whose return reports what.
8. **Reconcile GST allocation (Schedule D Part 2)** — If Schedule A Part 3 has entries and no election-out statement is attached, Line 5 must show the automatic allocation and Line 8 (exemption remaining) must be reduced accordingly. Verify this reconciles across both spouses' returns for split gifts — each spouse's own exemption is separately consumed. If an election-out statement is attached, confirm it covers the intended transfers and the return is timely (the election is only effective on a timely filed return). For **direct skips** in Part 2, the §2632(b) election-out has its own mechanism — the checkbox on Schedule A Part 2 (column j on the current revision) or an attached statement; confirm the box matches the client's intent.
9. **Verify the tax computation (Schedule D Part 3 and Part 2 of page 1)** — Inclusion ratio computed correctly; taxable gifts × rate table = tax; applicable credit ties to the current year's basic exclusion amount; prior taxable gifts tie to prior-year 709s; charitable gifts properly deducted on Schedule A Part 4 (with the required attachment if a partial-interest or split-interest gift); $0 balance due only where expected. Prior-period integrity goes beyond "ties to prior 709s as filed": ask whether there are prior **unreported** gifts (a delinquent return may be needed), whether the IRS adjusted any prior return on exam or in correspondence, and confirm Schedule B applicable-credit amounts are **recomputed at current rates** rather than carried at historical credit figures. If this is the donor's **first 709**, Schedule B should be blank — confirm no prior gifts exist rather than assuming.
10. **Verify DSUE / Restored Exclusion (Schedule C)** — If Part I Line 20 is "Yes" (DSUE applied): applicable credit on Part 2 Line 7 comes from **Schedule C Line 5**, not the standalone basic exclusion; DSUE amount ties to the last deceased spouse's Form 706 with the portability election made, and the **first four pages of that Form 706 are attached**; the **last-deceased-spouse limitation** is respected (only the most recent deceased spouse's DSUE, plus any DSUE from prior spouses actually applied to gifts before the last spouse's death); if the predeceased spouse's Form 706 was examined, the DSUE is the amount as **adjusted on exam**, not as originally filed; DSUE previously applied on prior 709s carries forward consistently; any Restored Exclusion Amount (§2801 covered-gift restoration) is accounted for on Line 3.
11. **Verify adequate disclosure** — For every hard-to-value gift: qualified appraisal attached, trust agreement attached, description sufficient under Treas. Reg. §301.6501(c)-1(f). This is what starts the statute of limitations; a return without it leaves the valuation open forever. If a **valuation discount** is claimed, confirm the Schedule A Line A box is checked and an attachment shows the discount computation and its support — both are part of adequate disclosure for discounted interests. Also consider recommending affirmative disclosure of **non-gift transactions** (intra-family sales, sales to trusts) to start the statute on recharacterization — a planning judgment; route the substantive call through `tax-advisor`.
12. **Verify timeliness** — If the transmittal cites an extended due date, confirm Form 4868/8892 was actually and timely filed, and that a copy is **attached to the return**, not just filed. Don't trust CCH Axcess's "extended" checkbox. If the donor **died during the gift year**, the 709 is due by the *earlier* of the normal due date or the Form 706 due date (with extensions) — a death-year return filed by April 15 can still be late, silently voiding the timely-filing elections this review polices.
13. **Summarize findings** — Produce the severity-graded report (see Output Format).
14. **Audit risk assessment** — Note 1–3 items presenting elevated audit risk, stated as facts: "This item may draw scrutiny because [specific reason]."
...
- Both spouses funded mirror trusts for each other in the same year — **reciprocal trust doctrine** exposure (the trusts may be uncrossed, putting the assets back in each grantor's estate); surface it and hand substantive analysis to `tax-advisor`
...
- Source documents show a bargain sale, family loan, debt forgiveness, entity capital shift, or premiums paid on a policy the donor doesn't own, with nothing corresponding on Schedule A — possible unreported deemed gift
- A life insurance **policy** gifted with no Form 712 attached, or a gifted policy funded through a split-dollar arrangement
- A lifetime QTIP election with no evidence the reverse QTIP election (§2652(a)(3)) was considered — the donor's GST exemption may be stranded
- Valuation discounts claimed without the Schedule A Line A box checked or a discount computation attached — the adequate-disclosure defect leaves the valuation open forever
- Gift-splitting mechanics applied to gifts funded from a community-property account (or community treatment assumed for separate property)
- A §2503(e) exclusion claimed for amounts paid to the student rather than directly to the institution, or for room and board
...
- **Substantive research** on whether a position is defensible (is this really a GST trust; should the client elect out; is a discount supportable; reciprocal trust doctrine exposure on paired SLATs; §2701/§2702 valuation of retained-interest transfers) → `tax-advisor`
